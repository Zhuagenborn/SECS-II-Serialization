...
public:
     ...
     */
    static std::expected<std::pair<Message, std::size_t>,
                         std::pair<std::errc, std::string>>
        BuildFromBytes(std::span<const std::byte>) noexcept;
...
std::expected<std::pair<Message, std::size_t>,
              std::pair<std::errc, std::string>>
    BuildMsgFromBytes(std::span<const std::byte>) noexcept;
...
public:
    ...
    auto format(const secs2::Message& msg,
...
public:
    auto format(const secs2::Type type,
                std::format_context& ctx) const noexcept;
};