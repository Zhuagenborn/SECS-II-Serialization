...
Message::BuildFromBytes(const std::span<const std::byte> bytes) noexcept {
    return BuildMsgFromBytes(bytes);
}

std::expected<std::pair<Message, std::size_t>,
              std::pair<std::errc, std::string>>
BuildMsgFromBytes(const std::span<const std::byte> bytes) noexcept {
...
auto std::formatter<secs2::Message>::format(
    ...
    return std::formatter<std::string>::format(
        indent_width_.has_value() ? msg.ToSml(indent_width_.value())
                                  : msg.ToSml(),
        ctx);
}
